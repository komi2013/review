...
class Kohana_Database_PostgreSQL extends Database
{
	...
	{
// 		var_dump($this->_config['connection']['info']);
//     var_dump(PGSQL_CONNECT_FORCE_NEW);
		if ($this->_connection)
			return;
		...
		try
		{
			$this->_connection = empty($this->_config['connection']['persistent'])
				? pg_connect('host=localhost '.$this->_config['connection']['info'], PGSQL_CONNECT_FORCE_NEW)
				: pg_pconnect('host=localhost '.$this->_config['connection']['info'], PGSQL_CONNECT_FORCE_NEW);
		}
		catch (ErrorException $e)
		{
			throw new Database_Exception(':error', array(':error' => $e->getMessage()));
		}
//     $this->_connection = pg_connect($this->_config['connection']['info']);
