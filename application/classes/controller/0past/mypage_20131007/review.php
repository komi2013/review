...
class Controller_Mypage_Review extends Controller_All {
	public function action_index()
	{
    parent::all();
		if (!$this->session->get('pv_u_id')) die('you must login');
    $u_id = $this->session->get('u_id');
    $enqu_id = isset($_GET['enqu_id']) ? $_GET['enqu_id'] : die('no enqu_id arg');
    if (!preg_match("/^[0-9]+$/", $enqu_id)) die('hey! what are you trying to do? no string');
    ...
    $this->page_title = $enquete->title;
		$question = MyORM::factory('question')->where('enqu_id', '=', $enqu_id)->order_by('q_num')->find_all();  
    $this->qu_arr = array();
    ...
    foreach ($question as $k => $d)
    {
      ...
      $qucho = MyORM::factory('qucho')->where('p_qu_id', '=', $d->id)->order_by('id')->find_all();
      $blank_cho = 0;
    ...
    $this->blank_question(6);
		...
		$this->template->content = View::factory('mypage/review');
    ...
    $this->template->content->enqu_id = $enqu_id;
	}
